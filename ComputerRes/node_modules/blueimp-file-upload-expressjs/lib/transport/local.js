    ...
    var api = {
        ...
        post: function(fileInfo, file, finish) {
            var me = this,
                options = this.options,
                versionFuncs = [],
                filepath = options.uploadDir + '/' + fileInfo.name;
            fs.renameSync(file.path, filepath);
            // var extname = path.extname(filepath);
            // console.log("path:  "+options.unzipPath);
            // if (extname=='.zip' && options.unzipName){
            //     var newPath = options.unzipPath+"/"+options.unzipName;
            //     console.log(newPath);
            //     fs.createReadStream(filepath).pipe(unzip.Extract({ path: newPath}));
            // }
                ...
                if (fileName[0] !== '.') {
                    fs.unlink(options.uploadDir + '/' + fileName, function(ex) {
                        Object.keys(options.imageVersions).forEach(function(version) {
                            ...
                            var thumbnailfile = options.uploadDir + '/' + version + '/' + fileName;
